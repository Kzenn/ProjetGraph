...
int main()
{
    ...
    BITMAP* edge_bmp = load_bitmap("edge.bmp",NULL);
    ...
    stabilo = load_sample("stabilo.wav");
    nextturn = load_sample("nextturn.wav");
    ...
    while(choix==1)
    {
        ...
        if(choix==3)
        {
            ...
            while(choix==3)
            {
                ...
                {
                    int x = ((elem.geta().getpos_x()+TAILLEX/2)+(elem.getb().getpos_x()+TAILLEX/2))/2;
                    int y = ((elem.geta().getpos_y()+TAILLEY/2)+(elem.getb().getpos_y()+TAILLEY/2))/2;
                    //int x = ((elem.geta().getpos_x()+TAILLEX/2)+x_temp)/2;
                    //int y = ((elem.geta().getpos_y()+TAILLEY/2)+y_temp)/2;
                    if(elem.getWeight()>0 && elem.geta().getpop()>0 && elem.getb().getpop()>0 && connexe == 0)
                    {
                        if(elem.getWeight()<51)
                        {
                            ...
                            if(elem.getDirection()==false)
                                thick_line(buffer,(elem.getb().getpos_x()+TAILLEX/2),(elem.getb().getpos_y()+TAILLEY/2),x,y,3.0,makecol(0,255,0));
                            else if(elem.getDirection()==true)
                                thick_line(buffer,(elem.geta().getpos_x()+TAILLEX/2),(elem.geta().getpos_y()+TAILLEY/2),x,y,3.0,makecol(0,255,0));
                        }
                        else if (elem.getWeight()>50 && elem.getWeight()<251)
                        {
                            ...
                            if(elem.getDirection()==false)
                                thick_line(buffer,(elem.getb().getpos_x()+TAILLEX/2),(elem.getb().getpos_y()+TAILLEY/2),x,y,8.0,makecol(0,255,0));
                            else if(elem.getDirection()==true)
                                thick_line(buffer,(elem.geta().getpos_x()+TAILLEX/2),(elem.geta().getpos_y()+TAILLEY/2),x,y,8.0,makecol(0,255,0));
                        }
                        else if (elem.getWeight()>200 && elem.getWeight()<501)
                        {
                            thick_line(buffer, elem.geta().getpos_x()+TAILLEX/2, elem.geta().getpos_y()+TAILLEY/2, elem.getb().getpos_x()+TAILLEX/2, elem.getb().getpos_y()+TAILLEY/2, 10.0, makecol(255, 255, 255));
                            if(elem.getDirection()==false)
                                thick_line(buffer,(elem.getb().getpos_x()+TAILLEX/2),(elem.getb().getpos_y()+TAILLEY/2),x,y,13.0,makecol(0,255,0));
                            else if(elem.getDirection()==true)
                                thick_line(buffer,(elem.geta().getpos_x()+TAILLEX/2),(elem.geta().getpos_y()+TAILLEY/2),x,y,13.0,makecol(0,255,0));
                        }
                        else
                        {
                            thick_line(buffer, elem.geta().getpos_x()+TAILLEX/2, elem.geta().getpos_y()+TAILLEY/2, elem.getb().getpos_x()+TAILLEX/2, elem.getb().getpos_y()+TAILLEY/2, 16.0, makecol(255, 255, 255));
                            if(elem.getDirection()==false)
                                thick_line(buffer,(elem.getb().getpos_x()+TAILLEX/2),(elem.getb().getpos_y()+TAILLEY/2),x,y,19.0,makecol(0,255,0));
                            else if(elem.getDirection()==true)
                                thick_line(buffer,(elem.geta().getpos_x()+TAILLEX/2),(elem.geta().getpos_y()+TAILLEY/2),x,y,19.0,makecol(0,255,0));
                        }
                    }
                    if(connexe==0)
                    {
                        ...
                        textprintf_ex(buffer,font,(abs(elem.geta().getpos_x()+elem.getb().getpos_x())/2)+18,(abs(elem.geta().getpos_y()+elem.getb().getpos_y())/2)+15,makecol(0,0,0), -1,  "%d", elem.getWeight());
                    }
                }

                if(connexe==1)
                    g.Affiche_composantes_FC(buffer);
                ...
                draw_sprite(buffer,conn_ic,15,SCREEN_H/1.45);
                ...
                for(int i=0; i<g.getSommetTab().size(); i++)
                {
                    if(d_vertex==0 && d_edge==0 && a_edge==0 && mouse_b&2 && mouse_x>g.getSommetTab()[i]->getpos_x()-30 && mouse_x<g.getSommetTab()[i]->getpos_x() && mouse_y>g.getSommetTab()[i]->getpos_y()+25 && mouse_y<g.getSommetTab()[i]->getpos_y()+80)
                    {
                        ...
                        if(g.getSommetTab()[i]->getpop()-1==0)
                        {
                        ...
                        else if(g.getSommetTab()[i]->getpop()-1<=0)
                        {
                            g.getSommetTab()[i]->setpop(0);
                            for(auto& elem : g.getEdgeTab())
                            {
                        ...
                        {
                            g.getSommetTab()[i]->setpop(g.getSommetTab()[i]->getpop()-1);
                            for(auto& elem : g.getEdgeTab())
                            {
                                if(g.getSommetTab()[i]->getchemin() == elem.geta().getchemin())
                                    elem.geta().setpop(g.getSommetTab()[i]->getpop());
                ...
                for(auto& elem : g.getEdgeTab())
                {
                    if(d_vertex==0 && d_edge==0 && a_edge==0 && mouse_b&2 && click==0 && mouse_x>(abs(elem.geta().getpos_x()+elem.getb().getpos_x())/2)+15 && mouse_x<(abs(elem.geta().getpos_x()+elem.getb().getpos_x())/2)+45 && mouse_y>(abs(elem.geta().getpos_y()+elem.getb().getpos_y())/2)+10 && mouse_y<(abs(elem.geta().getpos_y()+elem.getb().getpos_y())/2)+30)
                    {
                        ...
                        if(elem.getWeight()-1<=0)
                        {
                            elem.setWeight(0);
                            rest(100);
                        }
                        else if(elem.getWeight()-1>0)
                        {
                ...
                {
                    play_sample(nextturn,255, 128, 1000, false);
                    rest(200);
                    d_vertex=1;
                }
                if(d_vertex == 1)
                ...
                ///ON CLIQUE SUR STABILO ==> CONNEXE
                if(d_vertex==0 && d_edge==0 && a_edge==0 && mouse_x>15 && mouse_x<75 && mouse_y>(SCREEN_H/1.45) && mouse_y<(SCREEN_H/1.45)+60 && mouse_b&1)
                {
                    play_sample(stabilo,255, 128, 1000, false);
                    g.BuildAdjMatrix();
                ...
                if(g.getEdgeTab().size()!=0 && g.getSommetTab().size()!=0 && d_vertex==0 && d_edge==0 && a_edge==0 && mouse_x>15 && mouse_x<75 && mouse_y>(SCREEN_H/1.25) && mouse_y<(SCREEN_H/1.25)+60 && mouse_b&1)
                {
                    g.SaveFile(choice);
                ...
                ///ON CLIQUE SUR EXIT
                if(d_vertex==0 && d_edge==0 && a_edge==0 && mouse_x>15 && mouse_x<75 && mouse_y>(SCREEN_H/1.1) && mouse_y<(SCREEN_H/1.1)+60 && mouse_b&1)
                {
                    play_sample(nextturn,255, 128, 1000, false);
                    choix=1;
                    connexe=0;
